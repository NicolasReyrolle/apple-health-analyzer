...
class TestExportParser:
    ...
    def test_init(self, sample_file: str):
        ...
        assert list(parser.running_workouts.columns) == ["startDate", "endDate", "duration"]
...
"""
        ...
        # Should have loaded 2 running workouts (cycling should be filtered out)
        assert len(parser.running_workouts) == 2
        assert list(parser.running_workouts["startDate"]) == ["2024-01-01", "2024-01-03"]

    def test_load_running_workouts_empty(self, tmp_path: Path, capsys: CaptureFixture[str]) -> None:
...
"""
        ...
        assert len(parser.running_workouts) == 2
        # Check duration values are captured
