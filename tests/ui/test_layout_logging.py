...
def test_refresh_data_logs_to_root_logger(
    ...
    """Test that refresh_data calls logging and messages are captured."""
    # Setup mock workouts
    ...
    try:
        ...
        # Check if any log message contains "refresh_data" or "TEST"
        log_messages = [record.message for record in caplog.records]
        print(f"Captured log messages: {log_messages}")

        # Should have at least one message about refreshing
        assert any(
            "refresh_data" in msg or "TEST" in msg for msg in log_messages
        ), f"Expected refresh_data log message, got: {log_messages}"
